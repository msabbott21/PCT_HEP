...
def load_bes(h5_filenames,nevts=-1):
  ...
  data["top"] = np.concatenate([f['TopFrame_PFcands'][:nevts][...,[0,9,6,7,2,3,5,1]] for f in fs])
  ...
  #print("Make label")
  label = np.concatenate([np.full(len(f['BES_vars'][:nevts]),i) for i,f in enumerate(fs)])
