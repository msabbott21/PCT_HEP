...
def pairwise_distanceR(point_cloud,mask=None):
  ...
  Args:
    point_cloud: tensor (batch_size, num_points, 3)
...
if __name__ == "__main__":
    ...
    with tf.Graph().as_default():
      ...
      pair,mask_matrix = pairwise_distanceR(pointclouds_pl[:,:,:3],mask_zeros)
      nn_idx = knn(pair, k=k)
      with tf.Session() as sess:
        ...
        idxs,pairs,masks = sess.run([nn_idx,pair,mask_matrix], feed_dict=feed_dict)
        print (masks)
