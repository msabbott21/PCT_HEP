...
def PCT(besflag, input_global,npoints,nvars=1,nclass=2,nheads=1):
...
def PCT_simple(besflag, input_global,npoints,nvars=1,nclass=2,nheads=1):
    inputs = Input((npoints,nvars))
    batch_size = tf.shape(inputs)[0]
...
def newBESTNN(num_bes, nclass):
    # Create the BES framework

    # Input variables, shape = number of 'True' entries in mask
    besInputs = Input((num_bes,))        

    # Add BES variables to the network
    # The network architecture consists of 3 hidden layers with 100 nodes in each layer using a rectified-linear activation function.
    combLayer   = Dense(100, kernel_initializer="glorot_normal", activation="relu"   )(besInputs)
    combLayer   = Dense(100, kernel_initializer="glorot_normal", activation="relu"   )(combLayer)
    combLayer   = Dense(100, kernel_initializer="glorot_normal", activation="relu"   )(combLayer)
    
    outputs = Dense(nclass, kernel_initializer="glorot_normal", activation="softmax")(combLayer)

    return besInputs, outputs
